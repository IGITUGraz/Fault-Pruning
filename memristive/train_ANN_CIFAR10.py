...
from memristor_utils import set_R_to_get_w, generate_faults, update_memristive_vars
...
def prune(buffer, buffer_noisy, mask, threshold, stepwise_removal_percentage, min_connectivity,
    ...
    for i in idx_flattened:
        if len(buffer[i]) >= history_window:
            ...
            a = fit_line_no_intercept(np.array(buffer[i]), np.array(buffer_noisy[i]))  # estimated fault factor
            a_arr.append(a)
            i_arr.append(i)

    if len(a_arr) > 0:  # sort based on values of estimated fault factors, and threshold
        ...
        for cnt, k in enumerate(i_arr[sorting_key[key]]):
            ...
            f = ffs[k]  # ffs are "true fault factors", they should not be used in this function,
            # except to check if the alg. is performing well
            if f > 0.5:
                removed_wrong += 1

            if mask[idx] == 1:  # check constraints
                if (removed + 1) / total > stepwise_removal_percentage:  # not removing too many mem. in a single step
                    continue
                if (np.sum(mask) - 1) / total < min_connectivity:  # preserving min. connectivity
                    continue
...
def main(config):
    ...
    # You may crop CIFAR10 dataset to n-classes to run faster
    n_classes = 10
    x_train = x_train[y_train[:, 0] < n_classes, :, :, :]
    x_test = x_test[y_test[:, 0] < n_classes, :, :, :]
    y_train = y_train[y_train[:, 0] < n_classes]
    y_test = y_test[y_test[:, 0] < n_classes]
    ...
    # Configure the convolution and output layers
    # Architecture from: https://www.tensorflow.org/tutorials/images/cnn
    h1 = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), activation="relu")(input_image)
    h1 = tf.keras.layers.MaxPooling2D((2, 2))(h1)
    h2 = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), activation="relu")(h1)
    h2 = tf.keras.layers.MaxPooling2D((2, 2))(h2)
    h3 = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), activation="relu")(h2)
    h4 = tf.keras.layers.Flatten()(h3)
    h5 = tf.keras.layers.Dense(units=64, activation='relu')(h4)
    y_pred = tf.keras.layers.Dense(units=n_classes)(h5)

    # Construct the model
    model = tf.keras.Model(input_image, y_pred)
    model.summary()

    # Define a learning rate schedule and an optimizer
    lr_values = [5e-3, 1e-4, 1e-5]
    lr_boundaries = [16000,  # 15000
                     30000]  # boundaries are in terms of iterations (will roughly correspond to 40. & 80. epochs)
    ...
    # Define a loss function and compile model
    ...
    if config.simulate_faults:
        ...
        fault_factor = np.ones(n_weights, dtype=float)  # no faults
    ...
    stepwise_removal_percentage = 0.5  # 0.05
    ...
    for epoch in range(config.epochs):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            if step >= len(x_train) / config.batch_size:
                break
            ...
            if train_it % config.accumulative_update_iter == 0 or train_it == last_iter_idx:
                # w --> R --> R_noisy --> w_noisy
                w_new = model.get_weights()[::2]
                # if train_it % 100 == 0:
                #     for j in range(len(w_new)):
                #         print('Layer ' + str(j), np.min(w_new[j]), np.max(w_new[j]), np.min(R[j]), np.max(R[j]))
            ...
            # Log every x batches.
            if train_it % 50 == 0 or train_it == last_iter_idx:
                connectivity = np.sum(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--use-fixed-w", type=int, default=1,
    ...
    parser.add_argument("--using-laptop", type=int, default=1, help="Running on my laptop? (otherwise on office PC). "
                                                                    "When running on laptop, simulation data (acc, loss...) not saved.")
    ...
    percentage_stuck_lst = [0.1]
    percentage_negative_change_lst = [0.1]
        ...
        for perc_stuck in percentage_stuck_lst:
            for perc_neg in percentage_negative_change_lst:
                ...
                config = dict(
                    ...
                    delta_scaling_factor=delta,
                    percentage_stuck=perc_stuck,
                    percentage_negative_change=perc_neg,
                    history=args.history,  # history "type", based on dw or dR
                ...
                if config.to_prune == 0:
                    subname = '/CIFAR-10/to-prune-0'
                    # subname = '/CIFAR-10/to-prune-0-analysis'
                else:
                    subname = '/CIFAR-10/to-prune-1'
                    # subname = '/CIFAR-10/to-prune-1-analysis'
